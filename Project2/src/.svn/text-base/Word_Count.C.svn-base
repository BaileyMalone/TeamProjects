...
void Word_Count::add_word( std::string word )
{
  ...
  if(count)
    (*count)++; //Increment count if it exists already
  else
    hashTable.Add( word, 1 ); //Create new entry in hashTable with Word and count 1, since it's the first occurence.
}

int Word_Count::get_word_count( string word )
{
  int* count = hashTable.Get(word);
  if(count) //If count exists, return.
    return *count;
  else
    return 0; // Return 0 if word doesn't exist in Hash.
}
